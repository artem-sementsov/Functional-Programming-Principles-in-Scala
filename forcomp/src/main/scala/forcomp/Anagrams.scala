...
object Anagrams extends App {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.map(char => char.toLower).groupBy(char => char).map(map => (map._1, map._2.length)).toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char: Char, count: Int) :: tail => {
      val results: List[Occurrences] = combinations(tail)
      (for {
        result <- results
        n <- 1 to count
      } yield (char, n) :: result
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap)((acc: Map[Char, Int], cur: (Char, Int)) => {
      if (cur._2 == acc(cur._1)) acc - cur._1
      else acc updated(cur._1, acc(cur._1) - cur._2)
    }
    ).toList.sorted
  }
